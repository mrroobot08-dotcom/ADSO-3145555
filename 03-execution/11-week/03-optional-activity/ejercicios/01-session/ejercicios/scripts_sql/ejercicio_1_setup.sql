...
BEGIN
    IF EXISTS (SELECT 1 FROM boarding_pass WHERE check_in_id = NEW.check_in_id) THEN
        RETURN NEW;
    END IF;

    v_boarding_pass_code := 'BP-' || replace(NEW.check_in_id::text, '-', '');
    v_boarding_pass_code := left(v_boarding_pass_code, 40);
    
    v_barcode_value := 'BAR-' || replace(NEW.check_in_id::text, '-', '') 
                       || '-' || to_char(NEW.checked_in_at, 'YYYYMMDDHH24MISS');
    v_barcode_value := left(v_barcode_value, 120);

    INSERT INTO boarding_pass (check_in_id, boarding_pass_code, barcode_value, issued_at)
    VALUES (NEW.check_in_id, v_boarding_pass_code, v_barcode_value, NEW.checked_in_at);
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM ticket_segment WHERE ticket_segment_id = p_ticket_segment_id) THEN
        RAISE EXCEPTION 'Ticket_segment no existe';
    END IF;

    IF EXISTS (SELECT 1 FROM check_in WHERE ticket_segment_id = p_ticket_segment_id) THEN
        RAISE EXCEPTION 'Check-in ya existe para este ticket_segment';
    END IF;
